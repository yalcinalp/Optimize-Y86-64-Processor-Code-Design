...
# First of all, we need to have the changes on the parts such as instr_valid, need_valC, mem_read, d_srcB, aluA and alu. These neccessaties of these have been expalined in the seq-full.hcl
# 
# Addition to that, managing special circumstances resulting from the pipeline concept is the important point of this part.
# 
# Firstly, we have to deal with the return-hazard-like hazard of the ijtab.
# Because the pipeline will have already fetched the instruction directly below the jtab and we are unsure which instruction needs to be executed after the jtab instruction precisely.
# 
# To manage this, it's essential to introduce a bubble in the decode phase and stall the fetch stage, ensuring the fetched instruction is held back and not executed until the return 
# reaches the write_back stage.
# 
# We also have to deal with the scenario in which both the load use hazard and the jtab's above hazard occur at the same time. This occurs when data is loaded into memory prior to a jtab or return. It starts 
# when an instruction that requires data from memory is placed in register and this instruction serves as the source for both the subsequent instruction and the return/jtab in the decode process. In that 
# scenario, we have to prioritize the load use hazard so that it will wait to execute until the memory data is retrieved. Then the return hazard will then be managed in the normal.  
# 
# Another scenario to handle involves simultaneous load use hazard and jtab's specific hazard. This occurs when data is loaded into memory just before a return or jtab. It's triggered when an instruction 
# requiring data from memory is queued in a register, serving as the source for the subsequent instruction, with a return/jtab in the decode phase. In such cases, priority should be given to resolving the load # use hazard, allowing time for memory data retrieval and subsequent execution. Following this, the return hazard is addressed as usual.
#/* $begin pipe-all-hcl */
####################################################################
...
wrdsig SHLT	'STAT_HLT'	# Halt instruction encountered
